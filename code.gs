...
function createSlidesFromRow(row) {
 ...
 // art category is added to the front of the file name
 // to help order the slides by category
 deck.setName(row[2] + " " + row[0] + " " + row[1]);
 ...
 deck.replaceAllText("{{artCategory}}", row[2]);
...
function onOpen() {
 ...
 SpreadsheetApp.getUi().createMenu("reflections")
   .addItem("Create Slides", "mailMergeCertificatesFromSheets")
   .addToUi();
}