function mailMergeCertificatesFromSheets() {
  ...
  for (var i = 0; i < sheetContents.length; i++) {
    ...
    // Check if a Google Slides link has already been created
    if (slidesUrl === "") {
      ...
      // Generate a new Google Slides link for the presentation
...
function onOpen() {
 ...
 SpreadsheetApp.getUi().createMenu("Create_slides")
